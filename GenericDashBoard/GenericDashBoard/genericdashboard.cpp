...
GenericDashBoard::GenericDashBoard(QWidget *parent) : QWidget(parent)
{
    ...
    setAttribute(Qt::WA_TranslucentBackground, true);   //设置控件任何不透明都是半透明的
}
...
void GenericDashBoard::drawScale(QPainter* painter)
{
    ...
    const double scaleNum = m_scaleMajor * m_scaleMinor;  //刻度数量
    const double rotate = (m_endAngle - m_startAngle) / scaleNum;  //每次旋转的角度
    ...
    painter->setBrush(Qt::transparent);  //透明的黑色(相当于透过墨镜去看)
    painter->drawArc(QRectF(-shadowRadius, -shadowRadius,  //画弧线
                            shadowRadius * 2, shadowRadius * 2),
                     0 + 3 * 16, (m_endAngle - m_startAngle) * 16 - 3 * 16 * 2);
    ...
    for (int i = 0; i <= scaleNum; ++i)
    {
        ...
        if (i > scaleNum * m_scalePercentage)  //判断是不是到了警告刻度区
            color = m_scaleEndColor;

        if (i % m_scaleMinor == 0)     // 大刻度
        {
            painter->setPen(QPen(color, 1.4 * lineWidth));
            painter->drawLine(QPointF(0, radius), QPointF(0, radius / 1.16));  //圆心在widget中间
        }
        else
        {
        ...
        painter->rotate(rotate);  //旋转相邻刻度之间的角度
    }
...
{
    QPolygon polygon;
    int radius = m_radius * 0.6;  //指针长度
    double rotate = m_startAngle + 1.0 * (m_endAngle - m_startAngle) / (m_maxValue - m_minValue) * (m_curValue-m_minValue); //指针应当旋转的角度
    int width = radius * 0.08;  //指针的宽度

    painter->save();

    // 绘制指针
    polygon.setPoints(3, -width, 0, width, 0, 0, radius); //(3个点，点1坐标，点2坐标，点3坐标)，此时是一个指向正下方的指针
    painter->rotate(rotate);    //将指向正下方的指针指向正确的位置
...
void GenericDashBoard::drawText(QPainter* painter)
{
    ...
    painter->drawText(QPoint(-w / 2, -m_radius / 4), text);
    painter->restore();

...
protected:
    void resizeEvent(QResizeEvent*);  //保证控件中的图形大小布局随控件大小变化
...
private:
    ...
    int barHeight;                  //水银柱高度
    int radius;                     //水银柱底部圆半径
    int targetX;                    //目标X坐标;温度计本体的中轴线,表示温度计在widget中的位置
    QRectF barRect;                 //柱状区域
    ...
    int tickFontSize = 15;  //刻度表字体大小，建议在15附近

private slots:
    void updateTimerout();  //定时器绘制动画
...
public Q_SLOTS:
    ...
    int getTickFontSize();   //获取当前刻度表的字体大小
    void setTickFontSize(int i);  //设置刻度表的字体大小
Q_SIGNALS:
